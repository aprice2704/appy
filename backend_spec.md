...
*   `%%% replace`, `%%% replace_symbol <name>`, `%%% replace_block <cond>`, `%%% replace_element <selector>`: Opens a search or targeting block.
...
**Targeting:** Go Functions, Methods, Structs, Interfaces, Vars, and Consts.
**Mechanism:** Uses `go/ast` to find the exact byte boundaries of a symbol and replaces its entirety. For receiver methods, the format is `Receiver.MethodName` (the parser is forgiving of missing `*` pointers).

### 3.3 `replace_block` (AST Aware)
**Targeting:** Go `if`, `for`, and `switch` blocks.
**Mechanism:** Uses `go/ast` to find a control flow block matching a specific condition string.
